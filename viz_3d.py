...
# TODO: Get rid of this dummy wrapper properly
class SceneVisualizer:

    def __init__(self, frame_size: Tuple[int, int] = (640, 480)):

        # frame_size is ignored, Open3D manages its own window

        self.open3d_viz = Open3DVisualizer()
        self.last_scene = []

        # dummy frame for compatibility (not used)
        self.frame_size = frame_size
        self.dummy_frame = np.zeros((frame_size[1], frame_size[0], 3), dtype=np.uint8)

        print("Note: 3D visualization uses a separate Open3D window")
        print("Call process_3d_updates() from main thread to update the view")

    def draw_scene(self, scene_objects: List[SceneObject]) -> np.ndarray:
        """
        Queue a 3D scene update. The actual update happens when process_3d_updates() is called from main thread.
        Returns dummy frame for compat
        """
        self.last_scene = scene_objects
        # queue update for main thread to process
        self.open3d_viz.queue_update(scene_objects)
        return self.dummy_frame

    def process_3d_updates(self) -> bool:
        """
        Process queued 3D updates. Also needs to be called from main thread.
        """
        return self.open3d_viz.process_updates()

    def reset_view(self):
        self.open3d_viz.reset_view()

    def close(self):
        self.open3d_viz.close()

    # dummy methods (for compat)
    def dpg_drag_start(self, *args, **kwargs):
        pass

    def dpg_drag_move(self, *args, **kwargs):
        pass

    def dpg_drag_end(self, *args, **kwargs):
        pass

    def dpg_on_mouse_wheel(self, *args, **kwargs):
        pass